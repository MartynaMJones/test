...
, loc
, sku_segments 
from cnhdnf_analyse_prod.u_sku_segments 
where 10000*year+100*month+day  
in (select distinct load_timestamp from cnhdnf_analyse_prod.u_sku_segments_nsaz_tbl) 
)
go
...
--- DONE ---
drop table logistics_dwlab_prod.vg_proj_latest
go
create table logistics_dwlab_prod.vg_proj_latest
...
, loc
, cast(ss as float) ss
, cast(totdmd as float) as totdmd
, cast(ignoreddmd as float) as ignoreddmd
from cnhdnf_analyse_prod.u_skuprojstaticone_nsaz_tbl
where 10000*year+100*month+day in (select max(10000*year+100*month+day) from cnhdnf_analyse_prod.u_skuprojstaticone_nsaz_tbl)
...
drop table logistics_dwlab_prod.vg_fcst_latest
go
create table logistics_dwlab_prod.vg_fcst_latest
with (clustered columnstore index,distribution = round_robin) as
as
(
select item
, skuloc loc
, cast(totfcst as float) as totfcst
from  cnhdnf_analyse_prod.dfutoskufcst_nsaz_tbl
where 10000*year+100*month+day  
in (select max(10000*year+100*month+day) from cnhdnf_analyse_prod.dfutoskufcst_nsaz_tbl)
...
(
select * 
from cnhdnf_analyse_prod.item 
where 10000*year+100*month+day 
in (select max(10000*year+100*month+day) from cnhdnf_analyse_prod.item)  
)
go
...
select loc
, descr 
from cnhdnf_analyse_prod.loc 
where 10000*year+100*month+day  in (select max(10000*year+100*month+day) from cnhdnf_analyse_prod.loc) 
and loc in ('1292' --Vangarde
, '2639','3353', '1216' -- 3 POC
, '1861','1366' -- Vagarde control stores
, '2781', '0343', '5298', '2189', '3227', '3298' -- 3 POC control stores
...
, loc
, oh 
from cnhdnf_analyse_prod.sku 
where 10000*year+100*month+day in (select max(10000*year+100*month+day) from cnhdnf_analyse_prod.sku)
and loc in ('1292' --Vangarde
, '2639','3353', '1216' -- 3 POC
, '1861','1366' -- Vagarde control stores
, '2781', '0343', '5298', '2189', '3227', '3298' -- 3 POC control stores
...
from cnhdnf_analyse_prod.u_po_sto 
where 10000*year+100*month+day 
in (select max(10000*year+100*month+day) from cnhdnf_analyse_prod.u_po_sto) 
...
, dest as loc
, sum(qty) as qty 
from cnhdnf_analyse_prod.recship 
where 10000*year+100*month+day in (select max(10000*year+100*month+day) from cnhdnf_analyse_prod.recship) 
...
when seg.sku_segments is null and itm.u_bu_desc = "BEAUTY" then 'Not on segmentation BEAUTY'
